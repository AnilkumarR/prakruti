...
Company : EBSL
Author : Karan
Module : Purchase Quotation Analysis
Class 1 : PrakrutiPurchaseOrderQuotationAnalysis
Class 2 : PrakrutiPurchaseQuatationAnalysisLine
Table 1 & Reference Id : prakruti_purchase_order_quotation_analysis,order_line
Table 2 & Reference Id : prakruti_purchase_quotation_analysis_line,purchase_line_id
Updated By : Karan 
...
class PrakrutiPurchaseOrderQuotationAnalysis(models.Model):
    ...
    _description = 'Purchase Quotation Analysis'
    _rec_name = 'qa_no'
    _order="id desc" 
    
    '''
    Its an unique autogenerated Quotation Analysis Number which will be in the format of
    Format : PQA\PRODUCT_TYPE\NUMBER\FINANCIAL YEAR
    Example : PQA\EXFG\0001\17-18
    Updated By : Karan
    ...
    def _get_auto(self):
        ...
        for temp in self :
            ...
            parent_invoice_id = 0
            for value in result: parent_invoice_id = value['autogenerate_purchase_quotation_analysis'];# Database Function :autogenerate_purchase_quotation_analysis
            ...
            for record in self :
                if temp.purchase_type.group_code:
                    style_format[record.id] ='PQA\\'+ temp.purchase_type.group_code+'\\'+str(auto_gen)+'\\'+str(display_present_year)+'-'+str(dispay_year)
                else:                        
                    style_format[record.id] ='PQA\\'+str(auto_gen)+'\\'+str(display_present_year)+'-'+str(dispay_year)
                cr.execute('''UPDATE 
                                    prakruti_purchase_order_quotation_analysis 
                              SET 
                                    qa_no = %s 
    ...
    total_amount = fields.Float(string='Total Amount',compute= '_compute_total_amount',digits=(6,3))
    gst_total_in_words= fields.Text(compute= '_gst_total_in_words',string='GST Total in Words')
    all_additional_charges = fields.Float(string="All Additional Charges",compute= '_compute_all_additional_charges')#SUM OF all charges like freight_charges
    ...
    total_taxable_value = fields.Float(string="Total Taxable",compute= '_compute_total_taxable')
    revise_status = fields.Selection([('revise_quotation_analysis','Revise Quotation Analysis'),('revise_done','Revise Done')],string= 'Revise Status')
    ...
    state = fields.Selection([
                ('analysis','Purchase Quotation Analysis Draft'),
                ('order','Purchase Order'),
                ('confirm','Purchase Order Confirmed'),
                ('reject','Purchase Order Rejected'),
                ('short_close','Purchase Order Short Closed')],default= 'analysis', string= 'Status')
    ...
    '''
    Can't able to delete this record
    '''            
    @api.multi
    def unlink(self):
        for order in self:
    ...
    '''
    Pass the values to Purchase Order Screen Based on 2 conditions if the scheduled date is over than remarks must be entered  
    '''
    @api.one
    @api.multi 
                    ...
                    for vendor in cr.dictfetchall():
                        ...
                        if(temp.vendor_id.id == vendor ):
                            ...
                            ebsl_id = self.pool.get('prakruti.purchase_order').create(cr,uid, {
                                ...
                                'coming_from':'Purchase Quotation Analysis',
    ...
    @api.one
    @api.multi
    def revise_quotation_analysis(self):
        cr = self.env.cr
        uid = self.env.uid
        ids = self.ids
        context = 'context'
        for temp in self:
            ...
            for item in temp.order_line:
                erp_id = self.pool.get('prakruti.purchase_quotation_analysis_line').create(cr,uid, {
                    ...
                    'requisition_line_id': item.requisition_line_id,
                    'slip_id':item.slip_id.id,
                    'revise_quotation_analysis_line':item.revise_quotation_analysis_line,
                    'purchase_line_id': ebsl_id
                    })
            cr.execute("UPDATE prakruti_purchase_order_quotation_analysis SET revise_status = 'revise_quotation_analysis',is_revise = 'True' WHERE id = %s",((temp.id),))
            cr.execute("UPDATE prakruti_purchase_quotation_analysis_line SET revise_quotation_analysis_line = 1 WHERE purchase_line_id = %s",((temp.id),))
        return {} 
    '''
    ...
    def revise_done(self):
        ...
        for temp in self:
            if temp.revise_remarks:
                if temp.revise_id:
                    cr.execute("UPDATE prakruti_purchase_quotation_analysis_line SET revise_quotation_analysis_line = 2 WHERE purchase_line_id = %s",((temp.id),))
                    cr.execute('''SELECT revise_purchase_quotation_analysis AS error_message FROM revise_purchase_quotation_analysis(%s,%s)''',((temp.id),(temp.pr_no),))
                    for line in cr.dictfetchall():
...
class PrakrutiPurchaseQuatationAnalysisLine(models.Model):
    ...
    _description = 'Purchase Quotation Analysis Line'  
    ...
    proportionate_amount_to_products = fields.Float(related='purchase_line_id.proportionate_amount_to_products', string="Proportionate Amount to Products")
    ...
    revise_quotation_analysis_line=fields.Integer(string= 'Revised Flag',default=0)
