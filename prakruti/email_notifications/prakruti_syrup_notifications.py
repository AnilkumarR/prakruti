...
class prakruti_syrup_notifications(osv.osv_memory):
    _name = "prakruti.syrup_notifications"
    _description = "Syrup Hours Notifications"  

    def send_mail_to_higher_authority(self, cr, uid, ids, cron_mode=True, context=None):        
        ids = 22
        email_obj  = self.pool.get('email.template')
        template_id=email_obj.search(cr, uid, [('name', '=', 'Syrup Hours Exceeds')], context=context)[0]
        cr.execute('''  SELECT  
                                prakruti_syrup.write_date,
                                CURRENT_TIMESTAMP,
                                product_product.name_template as subplant_id,
                                prakruti_batch_master.batch_no as batch_id,
                                CURRENT_TIMESTAMP-prakruti_syrup.write_date as delay_datetime,
                                cast(prakruti_syrup.write_date as time) as write_time,
                                cast(CURRENT_TIMESTAMP as time) as current_date,
                                cast(CURRENT_TIMESTAMP as time)-cast(prakruti_syrup.write_date as time) as delay_time,
                                (extract(epoch from (timestamp 'now()' - prakruti_syrup.write_date))/60)/60 as hours,
                                state
                        FROM 
                                prakruti_syrup JOIN 
                                product_product ON
                                product_product.id=prakruti_syrup.subplant_id JOIN
                                prakruti_batch_master ON
                                prakruti_batch_master.id=prakruti_syrup.batch_no JOIN
                                res_users ON 
                                prakruti_syrup.material_recieved_by = res_users.id JOIN 
                                res_partner ON res_users.partner_id = res_partner.id 
                        WHERE  
                                state='draft' AND (CAST('2017-12-16 00:00:00' as timestamp) =
                                prakruti_syrup.write_date) OR 
				
                                prakruti_syrup.write_date > CAST('2017-12-16 00:00:00' as timestamp)
                                    ''' )       
        for plant in cr.dictfetchall():
            ...
            hours = plant['hours']
            state = plant['state']
            material_recieved_by = plant['material_recieved_by']
            sub =  'Notifications From Syrup Hours Exceeds'          
