...
class prakruti_syrup_production_notifications(osv.osv_memory):
    _name = "prakruti.syrup_production_notifications"
    _description = "Syrup Production Notifications"  

    def send_mail_to_higher_authority(self, cr, uid, ids, cron_mode=True, context=None):        
        ids = 22
        email_obj  = self.pool.get('email.template')
        template_id=email_obj.search(cr, uid, [('name', '=', 'Syrup Production Hours Exceeds')], context=context)[0]
        cr.execute('''  SELECT  
                                prakruti_syrup_production.write_date,
                                CURRENT_TIMESTAMP,
                                prakruti_batch_master.batch_end_date,
                                product_product.name_template as subplant_id,
                                prakruti_batch_master.batch_no as batch_id,
                                res_partner.name as created_by,
                                CURRENT_TIMESTAMP-prakruti_syrup_production.write_date as delay_datetime,
                                cast(prakruti_syrup_production.write_date as time) as write_time,
                                cast(CURRENT_TIMESTAMP as time) as current_date,
                                cast(CURRENT_TIMESTAMP as time)-cast(prakruti_syrup_production.write_date as time) as delay_time,
                                (extract(epoch from (timestamp 'now()' - prakruti_syrup_production.write_date))/60)/60 as hours
                                 
                        FROM 
                                prakruti_sub_plant JOIN 
                                prakruti_syrup_production ON
                                prakruti_sub_plant.id = prakruti_syrup_production.subplant_id JOIN
                                prakruti_batch_master ON
                                prakruti_batch_master.id=prakruti_syrup_production.batch_id JOIN
                                product_product ON
                                prakruti_sub_plant.subplant_id = product_product.id JOIN
                                res_users ON 
                                prakruti_syrup_production.created_by = res_users.id JOIN 
                                res_partner ON res_users.partner_id = res_partner.id 
                        WHERE  
                                (CAST('2017-12-16 00:00:00' as timestamp) =
                                prakruti_batch_master.write_date) OR 
				
                                prakruti_batch_master.write_date > CAST('2017-12-16 00:00:00' as timestamp)
                                    ''' )       
        for plant in cr.dictfetchall():
            ...
            sub =  'Notifications From Syrup Production Hours Exceeds'          
...
prakruti_syrup_production_notifications()
