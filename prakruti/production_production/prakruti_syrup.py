...
Author: Induja
Module: Syrup 
Class 1: PrakrutiSyrup
Class 2: PrakrutiSyrupline
C;ass 3 : PrakrutiSyruplinePacking
Table 1 & Reference Id: prakruti_syrup ,grid_id,grid_id1
Table 2 & Reference Id: prakruti_syrup_line,main_id
Table 3 & Reference Id: prakruti_syrup_line_packing,packing_id
Updated By: Induja
...
class PrakrutiSyrup(models.Model):
    _name = 'prakruti.syrup'
    _table = 'prakruti_syrup'
    _description = 'Syrup '
    _order = 'id desc'
    _rec_name = 'subplant_id'
    
    
    grid_id = fields.One2many('prakruti.syrup_line','main_id')
    grid_id1 = fields.One2many('prakruti.syrup_line_packing','packing_id')
    ...
    actual_batch_size =fields.Float(string='Actual Batch Size', required=1)
    date= fields.Date(string = 'Date',default=fields.Date.today, required=1)
    assay_per = fields.Float(string='% Assay',digits=(6,3))
    label_claim= fields.Char(string='Label Claim')
    material_issued_by = fields.Many2one('res.users',string='Material Issued By',readonly=1)
    material_checked_by = fields.Many2one('res.users',string='Material Checked By',readonly=1)
    material_recieved_by = fields.Many2one('res.users',string='Material Recieved By',readonly=1)
    ...
    existing_batch_no = fields.Integer(string= 'Existing Batch No.',default=0,readonly=1)
    ...
    flag_count = fields.Char(string= ' Update Flag',default=0,readonly=1)
    grn_line = fields.One2many('prakruti.syrup_bom_grn_list_line', 'syrup_id',string='GRN line')
    ...
    def unlink(self):
        ...
        return super(PrakrutiSyrup, self).unlink()
    '''
   ...
   Listing  the Products
    ...
    def action_list_products(self):
        ...
        context = 'context'
        for temp in self:
            cr.execute("SELECT product_id,description,uom_id,prakruti_standard_product_line.standard_value as std_value FROM prakruti_standard_product_line INNER JOIN prakruti_standard_product ON prakruti_standard_product_line.standard_line_id = prakruti_standard_product.id INNER JOIN prakruti_syrup ON prakruti_standard_product.subplant_id = prakruti_syrup.subplant_id AND prakruti_standard_product.id = prakruti_syrup.std_batch_size_id WHERE prakruti_syrup.id = %s",((temp.id),))
            for line in cr.dictfetchall():
                ...
                grid_line_entry = self.pool.get('prakruti.syrup_line').create(cr,uid,{
            ...
            cr.execute('UPDATE prakruti_syrup SET flag_display_product = 1 WHERE prakruti_syrup.id = %s',((temp.id),))
            cr.execute("UPDATE prakruti_syrup SET flag_delete_product = 0 WHERE prakruti_syrup.id = %s",((temp.id),))
        return {}    
    '''
    ...
    def action_delete_products(self):
        ...
        context = 'context'
        for temp in self:
            cr.execute("DELETE FROM prakruti_syrup_line WHERE prakruti_syrup_line.main_id = %s", ((temp.id),))
            cr.execute("UPDATE  prakruti_syrup SET flag_delete_product = 1 WHERE prakruti_syrup.id = %s",((temp.id),))
            cr.execute('UPDATE prakruti_syrup SET flag_display_product = 0 WHERE prakruti_syrup.id = %s',((temp.id),))
        return {}
    '''
    ...
    def action_list_packing_products(self):
        ...
        for temp in self:
            ...
            for line in cr.dictfetchall():
                ...
                std_qty = line['std_value']
                actual_qty = line['std_value']
                grid_line_entry = self.pool.get('prakruti.syrup_line_packing').create(cr,uid,{
                    'product_id':product_id,
                    'description':description,
                    'uom_id':uom_id,
                    'std_wgt':std_qty,
            ...
            cr.execute('UPDATE prakruti_syrup SET flag_display_product1 = 1 WHERE prakruti_syrup.id = %s',((temp.id),))
            cr.execute("UPDATE prakruti_syrup SET flag_delete_product1 = 0 WHERE prakruti_syrup.id = %s",((temp.id),))
        return {}    
    '''
    ...
    def action_delete_packing_products(self):
        ...
        context = 'context'
        for temp in self:
            cr.execute("DELETE FROM prakruti_syrup_line_packing WHERE prakruti_syrup_line_packing.packing_id = %s", ((temp.id),))
            cr.execute("UPDATE  prakruti_syrup SET flag_delete_product1 = 1 WHERE prakruti_syrup.id = %s",((temp.id),))
            cr.execute('UPDATE prakruti_syrup SET flag_display_product1 = 0 WHERE prakruti_syrup.id = %s',((temp.id),))
        return {}
    '''
    ...
    '''
    Push the data to store Request
    '''
    @api.one
    @api.multi 
    def request_to_stores(self):
        ...
        for temp in self:
            if temp.existing_batch_no != temp.batch_no.id:
                ...
                cr.execute("SELECT count(id) as no_of_line,count(actual_weight) as actual_count  FROM prakruti_syrup_line WHERE main_id = %s",((temp.id),))
                for line in cr.dictfetchall():
                ...
                if actual_count == no_of_line:
                    ...
                    cr.execute("update prakruti_syrup set readonly_flag = 1,flag_count = 1,state ='sent_stores' where id=%s",((temp.id),))
                    cr.execute("update prakruti_syrup_line set readonly_grid = 1 where main_id = %s",((temp.id),))
                else:
    ...
    '''
    Push the packing  material data to store Request
    '''
    @api.one
    @api.multi 
    def request_packing_material_issue(self):
        ...
        for temp in self:
            ...
            if temp.existing_batch_no != temp.batch_no.id:
                ...
                cr.execute("update prakruti_syrup_line_packing set readonly_grid = 1 where packing_id=%s",((temp.id),))
                cr.execute("update prakruti_syrup set state ='packing_request',flag_count =2,packing_material_request_flag = 0 where id=%s",((temp.id),))
            else:
                raise UserError(_('Oops...! Please Select different Batch Number'))
        return {}
    '''
    Push the extra issue material to store requst
    '''
    @api.one
    @api.multi 
    def request_to_stores_for_extra_issue(self):
        ...
        for temp in self:
            ...
            if temp.existing_batch_no != temp.batch_no.id:
                print 'BATCH ID',temp.batch_no.id
                print 'COPIED BATCH NUMBER',temp.existing_batch_no
                cr.execute("SELECT extra_qty as extra_count FROM prakruti_syrup_line WHERE extra_qty > 0 AND main_id = %s",((temp.id),))
                for line in cr.dictfetchall():
                ...
                if extra_count:
                    ...
                    cr.execute("SELECT product_id,description,uom_id,extra_qty,prakruti_syrup_line.id FROM prakruti_syrup_line WHERE extra_qty > 0 AND main_id = %s",((temp.id),))
                ...
                cr.execute("update prakruti_syrup set state ='extra_issue' where id=%s",((temp.id),))
                cr.execute("update prakruti_syrup set flag_count = 3 where id=%s",((temp.id),))
                # UPDATE STATE BEFORE PACKING
                if temp.state == 'issued_quant':
                    cr.execute("update prakruti_syrup set packing_material_request_flag = 2 where id=%s",((temp.id),))
                else:
                    cr.execute("update prakruti_syrup set packing_material_request_flag = 0 where id=%s",((temp.id),))
                    
            else:
                raise UserError(_('Oops...! Please Select different Batch Number'))
            #template_id = self.pool.get('email.template').search(cr,uid,[('name','=','Sales Syrup Extra')],context=context)[0]
            #email_obj = self.pool.get('email.template').send_mail(cr, uid, template_id,ids[0],force_send=True)
        return {}
    
    '''
    Push the extra issue Packing  material to store requst
    '''
    @api.one
    @api.multi 
    def request_to_stores_for_extra_issue_packing(self):
        ...
        for temp in self:
            ...
            if temp.existing_batch_no != temp.batch_no.id:
                print 'BATCH ID',temp.batch_no.id
                print 'COPIED BATCH NUMBER',temp.existing_batch_no
                cr.execute("SELECT count(id) as extra_count FROM prakruti_syrup_line_packing WHERE (extra_qty > 0 OR extra_issued_weight >0) AND packing_id = %s",((temp.id),))
                for line in cr.dictfetchall():
                    extra_count = line['extra_count']
                if extra_count:
                    #Store Request Entry
                    ...
                    cr.execute("SELECT product_id,description,uom_id,extra_qty,prakruti_syrup_line_packing.id,extra_issued_weight FROM prakruti_syrup_line_packing WHERE (extra_qty > 0 OR extra_issued_weight >0) AND packing_id = %s",((temp.id),))
                    for item in cr.dictfetchall():
                        product_id = item['product_id']
                        description = item['description']
                        uom_id = item['uom_id']
                ...
                cr.execute("update prakruti_syrup set state ='extra_packing_issue' where id=%s",((temp.id),))
                cr.execute("update prakruti_syrup set flag_count = 4 where id=%s",((temp.id),))
            else:
                raise UserError(_('Oops...! Please Select different Batch Number'))
            #template_id = self.pool.get('email.template').search(cr,uid,[('name','=','Sales Syrup Extra Packing')],context=context)[0]
            #email_obj = self.pool.get('email.template').send_mail(cr, uid, template_id,ids[0],force_send=True)
        return {}
    '''
    Duplicate Bom
    ...
    def duplicate_bom(self):
        ...
        context = 'context'        
        for temp in self:
            d_request = self.pool.get('prakruti.syrup').create(cr,uid, {
                'date':temp.date,
                'subplant_id':temp.subplant_id.id,
                'batch_no':temp.batch_no.id,
            ...
            for item in temp.grid_id:
                grid_values = self.pool.get('prakruti.syrup_line').create(cr,uid, {
                    'product_id': item.product_id.id,
            ...
            for item in temp.grid_id1:
                grid_values2 = self.pool.get('prakruti.syrup_line_packing').create(cr,uid, {
                    'product_id': item.product_id.id,
                    'composition': item.composition,
                    'description':item.description,
                    'uom_id':item.uom_id.id,
    ...
    def action_syrup_product_id(self, cr, uid, ids, context=None):
        prakruti_syrup = self.browse(cr, uid, ids[0], context=context)
        for temp in prakruti_syrup:
            return {
                'name': ('Additional Output'),
                'view_type':'form',
                'view_mode':'form',
                'res_model': 'prakruti.syrup_wizard',
                'view_id':False,
                'type':'ir.actions.act_window',
                'target':'new',
                'context': {
                            'default_syrup_bom_id':prakruti_syrup.id,
                            'default_batch_no':prakruti_syrup.batch_no.id,
                            'default_subplant_id':prakruti_syrup.subplant_id.id,
                            }, 
                
                }
    
    def action_syrup_packing_product_id(self, cr, uid, ids, context=None):
        prakruti_syrup = self.browse(cr, uid, ids[0], context=context)
        for temp in prakruti_syrup:
            return {
                'name': ('Additional Output'),
                'view_type':'form',
                'view_mode':'form',
                'res_model': 'prakruti.syrup_packing_wizard',
                'view_id':False,
                'type':'ir.actions.act_window',
                'target':'new',
                'context': {
                            'default_syrup_packing_bom_id':prakruti_syrup.id,
                            'default_batch_no':prakruti_syrup.batch_no.id,
                            'default_subplant_id':prakruti_syrup.subplant_id.id,
                            }, 
    ...
    def return_to_stores(self):
        ...
        for temp in self:
            ...
            ids = self.ids
            context = {}
            cr.execute("SELECT weight_return  FROM prakruti_syrup_line WHERE main_id = %s",((temp.id),)) 
            for line in cr.dictfetchall():
                weight_return = line['weight_return']
            #cr.execute("SELECT weight_returned  FROM prakruti_syrup_line_packing WHERE packing_id = %s",((temp.id),)) 
            #for line in cr.dictfetchall():
                #weight_returned = line['weight_returned']
                if weight_return > 0:
                    cr.execute("SELECT issued_weight,weight_return,extra_issued_qty  FROM prakruti_syrup_line WHERE main_id = %s",((temp.id),))
                    for line in cr.dictfetchall():
                        ...
                        cr.execute('''UPDATE 
                                    ...
                                    FROM (
                                            SELECT 
                                                    weight_return,
                                                    product_id,
                                                    prakruti_syrup_line.issue_common_id ,
                                                    prakruti_syrup_line.id 
                                            FROM    prakruti_syrup_line JOIN 
                                                    prakruti_syrup ON 
                    ...
                    #cr.execute("SELECT issued_weight,weight_returned,extra_issued_qty  FROM prakruti_syrup_line WHERE main_id = %s",((temp.id),))
                    #for line in cr.dictfetchall():
                        #issued_weight = line['issued_weight']
                        #weight_returned = line['weight_returned']
                        #extra_issued_qty = line['extra_issued_qty']
                    #if (issued_weight >= weight_returned) or (extra_issued_qty>=weight_returned):
                        #cr.execute('''UPDATE 
                                            #prakruti_store_issue_item as a 
                                    #SET 
                                    ...
                                    #FROM (
                                            #SELECT 
                                                    #weight_returned,
                                                    #product_id,
                                                    #prakruti_syrup_line_packing.id 
                                            #FROM    prakruti_syrup_line_packing JOIN 
                                                    #prakruti_syrup ON 
                                                    #prakruti_syrup_line_packing.packing_id=prakruti_syrup.id 
                                            #WHERE 
                                                    #packing_id=%s
                                            #)as b 
                                    #WHERE 
            ...
            if len(temp.grid_id1) != 0 :
                ...
                for line in cr.dictfetchall():
                    ...
                    if weight_returned > 0:
                        cr.execute("SELECT issued_weight,weight_returned,extra_issued_qty  FROM prakruti_syrup_line_packing WHERE packing_id = %s",((temp.id),))
                        for line in cr.dictfetchall():
                            issued_weight = line['issued_weight']
                            weight_returned = line['weight_returned']
                            extra_issued_qty = line['extra_issued_qty']
                            ...
                            cr.execute('''UPDATE 
                                        ...
                                        FROM (
                                                SELECT 
                                                        weight_returned,
                                                        product_id,
                                                        prakruti_syrup_line_packing.issue_common_id,
                                                        prakruti_syrup_line_packing.id 
                                                FROM    prakruti_syrup_line_packing JOIN 
                                                        prakruti_syrup ON 
                                                        prakruti_syrup_line_packing.packing_id=prakruti_syrup.id 
...
class PrakrutiSyrupline(models.Model):
    _name = 'prakruti.syrup_line'
    _table = 'prakruti_syrup_line'
    _description = 'Syrup Line'
    
    main_id = fields.Many2one('prakruti.syrup', ondelete='cascade')
...
class PrakrutiSyruplinePacking(models.Model):
    _name = 'prakruti.syrup_line_packing'
    _table = 'prakruti_syrup_line_packing'
    _description = 'Syrup Packing Line'
    
    packing_id = fields.Many2one('prakruti.syrup', ondelete='cascade')
...
class PrakrutiSyrupBOMGrnListLine(models.Model):
    _name = 'prakruti.syrup_bom_grn_list_line'
    _table = 'prakruti_syrup_bom_grn_list_line'
    _description = 'Syrup BOM GRN Line'
    
    syrup_id = fields.Many2one('prakruti.syrup',string="Syrup BOM ID")
