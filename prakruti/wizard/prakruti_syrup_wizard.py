...
class prakruti_syrup_wizard(models.TransientModel):
    _name = 'prakruti.syrup_wizard'
    _table = 'prakruti_syrup_wizard'
    _description = 'Syrup Wizard'
    
    syrup_bom_id=fields.Integer(string='Syrup Id')
    ...
    syrup_bom_wizard_line=  fields.One2many('prakruti.syrup_wizard_line','syrup_bom_wizard_id',string= 'Syrup Line')       
    @api.one
    @api.multi
    def action_approve(self):
        ...
        total_issued_qty = 0
        for temp in self:
            if len(temp.syrup_bom_wizard_line) > 0:
                cr.execute('''  INSERT INTO prakruti_syrup_line(product_id,uom_id,description,std_wgt,composition,material_weight,actual_weight,extra_qty,grn_no,grn_list,ar_no,issued_weight,extra_issued_qty,weight_return,weighed_by,checked_by,main_id)
                                SELECT 
                                    prakruti_syrup_wizard_line.product_id,
                                    prakruti_syrup_wizard_line.uom_id,
                                    prakruti_syrup_wizard_line.description,
                                    prakruti_syrup_wizard_line.std_wgt,
                                    prakruti_syrup_wizard_line.composition,
                                    prakruti_syrup_wizard_line.material_weight,
                                    prakruti_syrup_wizard_line.actual_weight,
                                    prakruti_syrup_wizard_line.extra_qty,
                                    prakruti_syrup_wizard_line.grn_no,
                                    prakruti_syrup_wizard_line.grn_list,
                                    prakruti_syrup_wizard_line.ar_no,
                                    prakruti_syrup_wizard_line.issued_weight,
                                    prakruti_syrup_wizard_line.extra_issued_qty,
                                    prakruti_syrup_wizard_line.weight_return,
                                    prakruti_syrup_wizard_line.weighed_by,
                                    prakruti_syrup_wizard_line.checked_by,
                                    prakruti_syrup_wizard.syrup_bom_id as grid_id
                                FROM
                                    prakruti_syrup_wizard_line JOIN
                                    prakruti_syrup_wizard ON
                                    prakruti_syrup_wizard.id = prakruti_syrup_wizard_line.syrup_bom_wizard_id 
                                WHERE 
                                    prakruti_syrup_wizard.id = %s''',((temp.id),))
                
            else:
                raise UserError(_('Oops Something is Missing...'))
        return {}

class prakruti_syrup_wizard_line(models.TransientModel):
    _name = 'prakruti.syrup_wizard_line'
    _table = 'prakruti_syrup_wizard_line'
    _description = 'Syrup Wizard Line'

    
    
    syrup_bom_wizard_id = fields.Many2one('prakruti.syrup_wizard',string="Grid", ondelete='cascade')
    ...
    _sql_constraints = [        
        ('unique_product_id','unique(product_id, syrup_bom_wizard_id)', 'Item(s) should be Unique')
        ]
